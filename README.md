...
| `ignoreSameNameCSSFile`                 | Whether to ignore a CSS file when an SCSS, Sass, or Less file with the same basename exists in the same directory. Useful when preprocessors compile to a same-name CSS file. Default value is `true`.
