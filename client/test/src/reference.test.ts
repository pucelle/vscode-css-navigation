...
describe('Test Reference', () => {
	...
	it('Should not find reference for tag selectors', async () => {
		assert.ok((await searchReferences('html'))!.length === 1)
		assert.ok((await searchReferences('body'))!.length === 1)
	})
	...
	it.only('Should find references for class selectors even within nesting', async () => {
	...
	it('Should find reference in current HTML document, be aware this is not available by default', async () => {
		assert.ok(
			(await searchReferences('.css-class-in-style', htmlDocument))!.includes('css-class-in-style')
		)
		assert.ok(
			(await searchReferences('&-in-style', htmlDocument))!.includes('scss-class-in-style')
		)
	})
})
